package xyz.jekyllex.app.packagemanager
...
import xyz.jekyllex.app.packagemanager.Constants.Companion.PACKAGE_INSTALLED_ACTION
...
class PackageInstaller(val context: Context) : PackageDownloader.ErrorListener,
    PackageDownloader.ProgressListener, PackageDownloader.StartListener,
    PackageDownloader.CompleteListener {
        ...
        if (isInstallationOfApkAllowed()) {
            ...
            GlobalScope.launch(Dispatchers.Main) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                    ...
                    val permIntent = Intent(Settings.ACTION_MANAGE_UNKNOWN_APP_SOURCES, Uri.parse("package:xyz.jekyllex"))
    ...
    private fun installAPK(packageName: String) {
        ...
        if (isInstallationOfApkAllowed()) {
            ...
            GlobalScope.launch(Dispatchers.Main) {
                ...
                Toast.makeText(context, "Please allow installation from unknown sources for Termux.", Toast.LENGTH_SHORT).show()
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                    context.startActivity(Intent(Settings.ACTION_MANAGE_UNKNOWN_APP_SOURCES, Uri.parse("package:xyz.jekyllex")))
                }
            }
